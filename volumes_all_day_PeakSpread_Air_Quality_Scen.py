...
create_scenario(from_scenario = emmy.bank.scenario(9001), scenario_id = scen_id, scenario_title = "All day volumes",  overwrite = True)

links_dic = {}
#check emmy_24.py to see scenarios being returned. It only yields scenarios between 2000 and 3000
print "fetching"
for scenario in emmy.bank.scenarios():
    if 9001 <= scenario.number <= 9007: 
        for link in Emmys.fetch_links(emmy.bank, scenario):
...
export_link_shape(export_file = "link_shape_{}.txt".format(9001), selection = "all")
import_shape = my_modeller.tool("inro.emme.data.network.base.link_shape_transaction")
import_shape(transaction_file = "link_shape_{}.txt".format(9001))
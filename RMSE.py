...
class Praser(object):
    ...
    def check_freeway(self, row, counts, pm2):
        ...
        #If cell isn't null
        # print cell
        # print self.data[cell].value
        if self.data[cell].value:
            ...
            if int(self.data["{}{}".format(self.cutlines,row)].value.encode('utf8')[0:3]) == 111 and self.data["{}{}".format(pm2,row + 1)].value is not None:
                usi5  = self.data["{}{}".format(pm2,row)].value + self.data["{}{}".format(pm2,row + 1)].value
                self.rmse.get_highway((usi5 - self.data["{}{}".format(counts,row)].value) ** 2)
                self.rmse.get_high_counts(self.data["{}{}".format(counts,row)].value)
            else:
                self.rmse.get_highway((self.data["{}{}".format(pm2,row)].value - self.data["{}{}".format(counts,row)].value) ** 2)
                self.rmse.get_high_counts(self.data["{}{}".format(counts,row)].value)
        else:
            self.rmse.get_arterials((self.data["{}{}".format(pm2,row)].value - self.data["{}{}".format(counts,row)].value) ** 2)
            self.rmse.get_art_counts(self.data["{}{}".format(counts,row)].value)
    ...
    def check_counts(self, row):
        ...
        if isinstance(self.data[ne_c].value, (int,long,float)) and isinstance(self.data[ne_pm2].value, (int,long,float)):            
            if self.data[ne_c].value > 0 and self.data[ne_pm2].value > 0:
                self.check_freeway(row, self.ne_counts, self.ne_pm2)
        if isinstance(self.data[sw_c].value, (int,long, float)) and isinstance(self.data[sw_pm2].value, (int,long,float)):
            if self.data[sw_c].value > 0 and self.data[sw_pm2].value > 0:
                self.check_freeway(row, self.sw_counts, self.sw_pm2)

    #Checks if columns is valid            
    def check_row(self, cell):
        if cell.value.encode('utf8')[0:3].isdigit():            
...
def main():
    ...
    # des_excel = "I:/ModServStaff/jimenez/Bill RMSE/b.xlsx"

##############################################################
## 
## 24 Hr performance measure class
## Using Emmys' API it creates different graphs and tables for different performance measures
...
#Public Methods    
    ...
    def create(self, **kargs):
        for scen, bank_path in enumerate(kargs['banks']):
            self.__get_bank(bank_path)        
            d = []
                ...
                for link in self.__fetch_links(s.number):
                    if  link["@speed"] > 0 and self.__is_auto(link):  
                        ...
                        if link is not  None:
                            if measure == "V/C":
                                self.__v_c(link)                        
                                cname = ["Hour","V/C_Scen{}".format(scen + 1),"SEVERELY CONGESTED MILES_Scen{}".format(scen + 1)]                    
                            else:        
                                agr.append(self.functions[measure])
                                cname = ["Hour","{}_Scen{}".format(measure, scen + 1)]
            ...
            temp = DataFrame(d, columns = cname)       
    ...
    def get_all_vol(self, **kargs):
        self.__get_bank(kargs['path'])
        all_volume = defaultdict(list)
        for s in self.__fetch_scenario(self.bank):
            hour = self.__get_hour(s)
            link_sum = 0
            for link in self.__fetch_links(s.number):
                if self.__is_auto(link) and link["@speed"] > 0:
                    link_sum += link.auto_volume
            all_volume[hour] = link_sum
        with open("C:\\Users\\jimenez\\Desktop\out_vol{}.csv".format(kargs['year']), "wb") as f:
            writer = csv.writer(f)
            writer.writerow(["Hour", "Total_Volume"])
            for k,v in all_volume.iteritems():
                writer.writerow([k,v])
    ...
    def plot(self, **kargs):       
        columns = ["{}_Scen{}".format(kargs["measure"].upper(), i) for i in [1,2]]
        if kargs.get('speed'):            
        ...
        self.ax.text(.01, .86,kargs['description'], 
                  horizontalalignment='left',
                  verticalalignment='top',
                  transform = self.ax.transAxes,
                  fontsize = 8)
...
# private methods
    ...
    def get_links_cv(self, **kargs):
        ...
        if kargs.get('one_way'):
            ...
            one_way = False
            if kargs['dire'].upper() == "N":
                cnames = ["Hour","NB_PS","SB_PS"]
            elif kargs['dire'].upper() == "E": 
                cnames = ["Hour","EB_PS","WB_PS"]
        ...
        for s in self.__fetch_scenario(self.bank):
            ...
            for link in self.__fetch_links(s.number):                 
                if link.id in kargs['links']:
                    if one_way:
                        if link.volume_delay_func in [1,2,3,5,6,7]:  
                            volume += float(link.auto_volume) / link.data3 
                        elif link.volume_delay_func in [4,8]:
                            volume +=  float(link.auto_volume) / link["@mb"]
                    else:
                        if link.volume_delay_func in [1,2,3,5,6,7]:  
                            volume += float(link.auto_volume) / link.data3
                            r_volume += float(link.reverse_link.auto_volume)/ link.data3 
                        elif link.volume_delay_func in [4,8]:
                            volume +=  float(link.auto_volume) / link["@mb"]
                            r_volume += float(link.reverse_link.auto_volume) / link["@mb"]            
            if one_way:
                d.append([hour,volume/len(kargs['links'])])
            else:
                d.append([hour,volume/len(kargs['links']), r_volume/len(kargs['links'])])
        columns = self.__columns(kargs['assignment'], cnames)    
        temp = DataFrame(d, columns = columns)
        self.df = DataFrame.merge(self.df, temp, how="left", on="Hour")
